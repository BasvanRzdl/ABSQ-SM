...
from AspEntQuaNet.measures import kullback_leibler_divergence, relative_absolute_error, absolute_error
...
tuner = kt.Hyperband(build_model_BiLSTM,
                        ...
                        overwrite=False,
                        directory="logs/hp",
                        project_name="AspClassErrQuaNet",)
...
best_model.save_weights('/Users/basvanroozendaal/Downloads/DATA THESIS/Raw data/LCRRothopPP outputs restaurant 2016/AspClassErrQuaNet.h5')
...
x_test_classerr = create_trainings_data(all_data=test_results_lcr, sub_dataset_percentage=0.4, number_of_subdatasets=1000, sort_key=sort_key, subdataset_size=250)
y_test_true_classerr = x_test_classerr.pop('true_prevalence')

y_test_pred_classerr = best_model.predict(x_test_classerr)

kld_classerr = kullback_leibler_divergence(y_test_true_classerr, y_test_pred_classerr)
rae_classerr = relative_absolute_error(y_test_true_classerr, y_test_pred_classerr)
ae_classerr = absolute_error(y_test_true_classerr, y_test_pred_classerr)
print(f'KLD: {kld_classerr}, RAE: {rae_classerr}, AE: {ae_classerr}')

y_test_baselines = get_baseline_metrics(y_test_true_classerr, x_test_classerr)
